...
class FaqList extends Component
{
    ...
    {
        $faqMasterQuery = Faq::query();
        if ($this->searchQuestion)
            $faqMasterQuery->Where('question', 'like', '%' . $this->searchQuestion . '%');
        if ($this->searchAnswer)
            $faqMasterQuery->Where('answer', 'like', '%' . $this->searchAnswer . '%');
    ...
    {
        $faq->fill(['active' => ($faq->active == 1) ? 0 : 1])->save();
